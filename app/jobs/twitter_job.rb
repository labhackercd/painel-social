...
def twitter_process(search_term, slug)
  ...
  begin
    ...
    t = twitter.search(URI::encode(search_term), :result_type => "recent", :lang => "pt")
    for i in (0..2)
      t.each do | tweet |
        tweets.push(tweet)
      ...
      if i >= 1
        t = twitter.search(URI::encode(search_term), :result_type => "recent", :max_id => max_id, :lang => "pt")
      end
    end
    ...
    event = {value: wordcloud, moreinfo: 'Hashtags associados à busca "%s"<br />(baseado em %s tweets nas últimas %s horas)' % [search_term, tweets.count, hours]}
    ...
    if search_term.start_with?('@')
      moreinfo = 'Mensagens recentes no twitter direcionadas a %s<br />(baseado em %s tweets nas últimas %s horas)' % [search_term, tweets.count, hours]
    else
      moreinfo = 'Mensagens recentes no twitter relacionadas à busca "%s"<br />(baseado em %s tweets nas últimas %s horas)' % [search_term, tweets.count, hours]
    end
...
Dashing.scheduler.every '60m', :first_in => 0.4 do |job|
  twitter_process('@HenriqueEAlves', 'henriqueealves')
  twitter_process('câmara dos deputados', 'camaradosdeputados')
end
